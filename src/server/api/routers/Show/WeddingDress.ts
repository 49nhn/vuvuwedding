...
export const weddingDressRouter = createTRPCRouter({
    getList: AuthMiddleware
        ...
        .query(async ({ ctx, input }) => {
                ...
                const weddingDresses = await ctx.prisma.photo.findMany(
                    {
                ...
                if (!weddingDresses) throw new TRPCError({ code: "NOT_FOUND", message: "Photos not found" })
                return {
                    items:weddingDresses,
    ...
    delete: AuthMiddleware
        ...
        .mutation(async ({ ctx, input }) => {
                const decorations = await ctx.prisma.weddingDress.delete({
                    where: {
                        id: input,
                    },
                });
